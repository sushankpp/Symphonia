...
const RoleRequests: React.FC = () => {
  ...
  // Form state
  const [requestedRole, setRequestedRole] = useState<'artist' | 'admin'>('artist');
  ...
  const fetchRoleRequests = async () => {
    try {
      ...
      const validRequests = Array.isArray(data) ? data.filter(req => req && typeof req === 'object') : [];
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only add the new request if it's valid
  ...
  const handleCancelRequest = async (requestId: number) => {
    ...
    try {
      const updatedRequest = await roleRequestService.cancelRoleRequest(requestId);
      if (updatedRequest && typeof updatedRequest === 'object') {
        setRequests((Array.isArray(requests) ? requests : [])
          .filter(req => req && typeof req === 'object') // Filter out invalid items
  ...
  const canSubmitNewRequest = () => {
    ...
    const hasPendingRequest = requests.some(req => req && req.status === 'pending');
      ...
      {user && (
        <div className="current-role-info">
          <div className="role-card">
            ...
            <p>
              {user.role === 'user' && 'You can request to become an Artist or Admin'}
              {user.role === 'artist' && 'You can request to become an Admin'}
              {user.role === 'admin' && 'You have the highest level access'}
            </p>
          </div>
        </div>
      )}
            ...
            <div className="request-form-container">
              ...
              <form onSubmit={handleSubmitRequest} className="request-form">
                <div className="form-group">
                  ...
                  <select
                    value={requestedRole}
                    onChange={(e) => setRequestedRole(e.target.value as 'artist' | 'admin')}
                    required
                  >
                    {user?.role === 'user' && (
                      <>
                ...
                <div className="role-requirements">
                  ...
                  {requestedRole === 'artist' ? (
                    <ul>
                      <li>Must be a genuine music creator or performer</li>
                      <li>Should have music content to share</li>
                      <li>Committed to engaging with the community</li>
                    </ul>
                  ) : (
                    <ul>
                      <li>Must have relevant experience in music or administration</li>
                      <li>Should demonstrate leadership qualities</li>
                      <li>Committed to helping moderate and manage the platform</li>
                    </ul>
                  )}
                </div>
      ...
      {!canSubmitNewRequest() && (
        <div className="pending-request-notice">
          <div className="notice-content">
            ...
            <p>You have a pending role request. Please wait for admin review before submitting a new request.</p>
          </div>
        </div>
      )}
      ...
      <div className="requests-history">
        ...
        {loading ? (
          ...
          <div className="requests-list">
            {(requests || []).filter(req => req && typeof req === 'object').map((request) => (
      ...
      <div className="help-section">
        ...
        <div className="help-content">
          <div className="help-item">
            ...
            <p>Role requests are typically reviewed within 1-3 business days by our admin team.</p>
          </div>
          <div className="help-item">
            <h4>What happens after approval?</h4>
            <p>Once approved, your role will be automatically updated and you'll have access to new features.</p>
          </div>
          <div className="help-item">
            <h4>Can I resubmit after rejection?</h4>
            <p>Yes, you can submit a new request after addressing the feedback provided by admins.</p>
          </div>
        </div>
      </div>
          </div>
