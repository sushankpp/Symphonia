            ...
            artistWithSongs.forEach(item => {
                item.songs.forEach(currentSong => {
                    getAudioDuration(currentSong.file_path)
                        ...
                        .catch(error => {
                            console.error(`Error fetching duration for song ${currentSong.id}:`, error);
                        });
                });
            });
        }
        ...
        <>
            ...
            <main className="page__home" id="primary">
                <div className="container">
                    <TopHeader/>
                    {loading ? (
                        <p className="loading">Music Data are being loaded. Please wait....</p>
                    ) : error ? (
                        <p className="error-message">{error}</p>
                    ) : (
                        <div className="music-list__container">
                            {artistWithSongs.length > 0 ? artistWithSongs.map((item, index) => (
                                <div className="music-list__wrapper" key={index}>
                                    <ArtistDetails
                                        ...
                                        onPlayAllClick={() => console.log(`Play all songs for ${item.artist.artist_name}`)}
                                    />
                                    <SongsList
                                        ...
                                        onSongClick={(songId) => handleSongClick(item.artist.id, songId)}
                                    />
                                </div>
                            )) : (
